...
for seed in 0 1 2
do
    time python3 -u generate.py --model=$model --dataset=$dataset --eps=$eps --temp=$temp --batch=$batch --num=$num --seed=$seed --tokens=$toks $options
done 
done
done
done
...
for seed in 0 1 2
do
    time python3 -u generate.py --model=$model --dataset=$dataset --eps=$eps --temp=$temp --topk=$topk --batch=$batch --num=$num --seed=$seed --tokens=$toks $options
done 
done
done
done
